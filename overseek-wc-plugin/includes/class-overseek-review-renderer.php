...
class OverSeek_Review_Renderer {
	...
	private static function render_review_card( array $review, bool $show_product, bool $show_media, bool $show_replies = true, array $args = [] ): string {
		...
		$product_class = 'os-review-card__product' . ( '' === $product_image ? ' os-review-card__product--no-image' : '' );
		...
		ob_start();
		?>
		<article class="os-review-card">
			<header class="os-review-card__header">
				<div class="os-review-card__reviewer">
					<?php echo self::render_avatar( $reviewer, $args['avatars'] ?? 'initials' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					<div>
