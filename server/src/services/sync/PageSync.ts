...
export class PageSync extends BaseSync {
    ...
    protected async sync(woo: WooService, accountId: string, incremental: boolean, job?: any, syncId?: string): Promise<SyncResult> {
        ...
        while (hasMore) {
            ...
            if (!pages.length) {
                hasMore = this.hasMorePages(page, totalPages, rawPages.length, 50);
                if (job) {
            ...
            totalProcessed += pages.length - failedWooIds.length;
            hasMore = this.hasMorePages(page, totalPages, rawPages.length, 50);
        ...
        if (totalUpsertFailures > 0) {
            throw new Error(`Page sync could not persist ${totalUpsertFailures} page(s); checkpoint was not advanced.`);
        }
