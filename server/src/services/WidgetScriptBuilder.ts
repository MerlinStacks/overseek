/**
 * Widget Script Builder Service
 * 
 * Generates the embeddable chat widget JavaScript.
 * Extracted from widget.ts for maintainability.
 */
...
function generateStyles(config: WidgetConfig): string {
    const { position, showOnMobile } = config;
...
/**
 * Generates the complete widget script
 */
export function buildWidgetScript(config: WidgetConfig): string {
    const {
        apiUrl, accountId, primaryColor, headerText,
        welcomeMessage, businessHours, businessTimezone
    } = config;
...
(function() {
    ...
    var styles = ${JSON.stringify(generateStyles(config))};
