...
describe('settings activation intent', () => {
    it.each([['guarded', false], ['guarded', true], ['legacy', true]] as const)('revalidates %s transport with desiredActive=%s', async (cutoverState, desiredActive) => {
        const row: any = { desiredActive, cutoverState, receivingFrozen: false, controlAction: null, controlRevision: 5n, controlLeaseToken: 'existing-lease' };
        const updateMany = vi.fn(async ({ where, data }) => {
            if (row.desiredActive !== where.desiredActive || row.receivingFrozen !== where.receivingFrozen || !where.OR.some((c: any) => row.controlAction === c.controlAction)) return { count: 0 };
