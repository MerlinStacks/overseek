/**
 * Knowledge Module Index
 * 
 * AI Marketing Co-Pilot Knowledge Base and Recommendation Engine.
 */

export { MarketingKnowledgeBase } from './MarketingKnowledgeBase';
export type { AnalysisContext, KnowledgeEntry, MatchedRecommendation } from './MarketingKnowledgeBase';

export { RecommendationEngine } from './RecommendationEngine';
export type { ExplainableRecommendation, RecommendationSummary } from './RecommendationEngine';

export { RecommendationTracker } from './RecommendationTracker';
export type { RecommendationStatus, RecommendationFeedback, RecommendationStats } from './RecommendationTracker';
