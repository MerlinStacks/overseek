/**
 * KeywordTrackerPanel — Track specific keywords and view position history.
 *
 * Shows a list of tracked keywords with current metrics,
 * an add-keyword form, and a position history chart for the selected keyword.
 *
 * Why SVG for charts: avoids charting library dependency while still
 * providing interactive tooltips, gradient fills, and smooth rendering.
 */
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Tracked Keywords</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400">
                        {keywords.length} keyword{keywords.length !== 1 ? 's' : ''} being tracked
                    </p>
                </div>
            ...
            {showAddForm && (
                <>
                    ...
                    {addError && (
                        <p className="text-xs text-red-500 dark:text-red-400 px-1 -mt-4">{addError}</p>
                    )}
                </>
            )}

            {/* Suggested Keywords — sourced from low-hanging fruit + AI recs */}
            <SuggestedKeywords
                ...
                onTrack={async (keyword: string) => {
                    ...
                    } catch { /* swallow — user sees result in list */ }
                }}
                isAdding={addKeyword.isPending}
            />

            {/* Keyword List + History Layout */}
                ...
                <div className="xl:col-span-1 glass-panel rounded-2xl overflow-hidden">
                    {isLoading ? (
                        ...
                        <div className="text-center py-14 px-6">
                            ...
                            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Click "Track Keyword" to start monitoring positions.</p>
                        </div>
                    ) : (
