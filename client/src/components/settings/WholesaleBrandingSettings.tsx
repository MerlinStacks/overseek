    ...
    return <section className="border-t border-gray-100 pt-6 dark:border-slate-700">
        ...
        {loading ? <div className="flex items-center gap-2 py-6 text-sm text-gray-500"><Loader2 className="animate-spin" size={18} /> Loading catalog branding...</div> : <>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
                ...
                <Field label="Legal name" disabled={!canEdit} value={String(value.businessDetails.legalName || '')} onChange={legalName => setValue({ ...value, businessDetails: { ...value.businessDetails, legalName } })} />
                <Field label="Business number" disabled={!canEdit} value={String(value.businessDetails.businessNumber || value.businessDetails.abn || '')} onChange={businessNumber => setValue({ ...value, businessDetails: { ...value.businessDetails, businessNumber } })} />
                <Field label="Contact email" type="email" disabled={!canEdit} value={String(value.businessDetails.contactEmail || value.businessDetails.email || '')} onChange={contactEmail => setValue({ ...value, businessDetails: { ...value.businessDetails, contactEmail } })} />
                <Field label="Contact phone" disabled={!canEdit} value={String(value.businessDetails.contactPhone || value.businessDetails.phone || '')} onChange={contactPhone => setValue({ ...value, businessDetails: { ...value.businessDetails, contactPhone } })} />
                ...
                <label className="text-sm text-slate-600 dark:text-slate-300 sm:col-span-2">Legal notice<textarea disabled={!canEdit} rows={3} value={String(value.businessDetails.legalNotice || '')} onChange={event => setValue({ ...value, businessDetails: { ...value.businessDetails, legalNotice: event.target.value } })} className={inputClass} /></label>
            </div>
