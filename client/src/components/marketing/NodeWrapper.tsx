...
export const NodeWrapper: React.FC<NodeWrapperProps> = ({
    ...
    const issueSummary = issues.slice(0, 2).map((issue) => issue.message).join(' ');

    return (
    <div className="relative mb-8">
        <div className={`rounded-2xl border ${issues.length > 0 ? (blockingCount > 0 ? 'border-red-300' : 'border-amber-300') : borderColor} ${bgColor} ${isCompact ? 'min-w-[210px] max-w-[270px]' : 'min-w-[240px] max-w-[320px]'} overflow-hidden shadow-xl shadow-slate-900/8 ring-1 ring-slate-900/5 transition-shadow hover:shadow-2xl hover:shadow-slate-900/12`}>
            <div className={`flex items-center gap-3 ${isCompact ? 'px-3 py-2.5' : 'px-4 py-3'} border-b border-slate-100 bg-linear-to-r from-slate-50/80 to-white`}>
                ...
                {issues.length > 0 && (
                    <div title={issueSummary} className={`inline-flex items-center gap-1 rounded-full border px-2 py-1 text-[11px] font-semibold ${blockingCount > 0 ? 'border-red-200 bg-red-50 text-red-700' : 'border-amber-200 bg-amber-50 text-amber-700'}`}>
