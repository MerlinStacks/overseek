...
interface Props {
    ...
    onOpenSettings: () => void;
    onDropOnSection: (event: DragEvent, sectionId: string, insertIndex?: number) => void;
}
                ...
                {sections.map((section) => {
                    ...
                    return (
                        <section key={section.id} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { setDropTarget(null); onDropOnSection(event, section.id); }} onClick={() => onSelectSection(section.id)} className={`group/section relative transition ${selectedSectionId === section.id ? 'outline outline-2 outline-indigo-400' : 'outline outline-1 outline-transparent hover:outline-indigo-200'}`} style={sectionStyle}>
                            ...
                            {blocks.length === 0 && <div className="rounded-lg border border-dashed border-slate-300 px-3 py-8 text-center text-sm text-slate-400">Drag a block or structure into this section</div>}
                            {blocks.map((block, index) => (
                                <div key={block.id} className="group/block relative" draggable onDragStart={(event) => { event.dataTransfer.setData('application/x-overseek-existing-block', block.id); event.dataTransfer.effectAllowed = 'move'; }} onDragOver={(event) => { event.preventDefault(); setDropTarget(`${section.id}:${index}`); }} onDragLeave={() => setDropTarget(null)} onDrop={(event) => { setDropTarget(null); onDropOnSection(event, section.id, index); }} onClick={(event) => { event.stopPropagation(); onSelectSection(section.id); onSelectBlock(block.id); }}>
                                    <div className={`h-1 rounded-full transition ${dropTarget === `${section.id}:${index}` ? 'bg-indigo-500' : 'bg-transparent'}`} />
