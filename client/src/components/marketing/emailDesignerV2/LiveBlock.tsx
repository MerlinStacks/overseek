...
export function LiveBlock({ block, theme, onUpdate }: { block: EmailBlock; theme: EmailDesignTheme; onUpdate: (updater: (block: EmailBlock) => void) => void }) {
    ...
    if (block.type === 'footer') {
        return <div style={{ padding: block.props.padding || '8px 0', textAlign: block.props.align || 'center', fontSize: 12, lineHeight: 1.6, color: block.props.color || theme.mutedTextColor }} dangerouslySetInnerHTML={{ __html: sanitizeEmailHtml(block.props.html || '<p><a href="{{unsubscribe_url}}">Unsubscribe</a></p>') }} />;
    }
    ...
    if (block.type === 'newProducts') {
        const rawCount = Number(block.props.count);
        const rawColumns = Number(block.props.columns);
        const count = Number.isFinite(rawCount) ? Math.min(6, Math.max(1, Math.floor(rawCount))) : 3;
        const columns = Number.isFinite(rawColumns) ? Math.min(3, Math.max(1, Math.floor(rawColumns))) : 3;
        return <div style={{ padding: block.props.padding || '18px 0', textAlign: block.props.align || 'center', ...responsiveStyle }}>{block.props.heading && <h3 style={{ margin: '0 0 16px', color: theme.textColor, fontSize: 20, lineHeight: 1.3 }}>{block.props.heading}</h3>}<div style={{ display: 'grid', gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))`, gap: 12 }}>{Array.from({ length: count }).map((_, index) => <div key={index} style={{ border: '1px solid #e2e8f0', borderRadius: 10, padding: 12 }}>{block.props.showImage && <div style={{ height: 82, borderRadius: 8, background: '#f1f5f9', marginBottom: 10 }} /> }<p style={{ margin: '0 0 6px', color: theme.textColor, fontWeight: 700 }}>Newest product {index + 1}</p>{block.props.showDescription && <p style={{ margin: '0 0 8px', color: theme.mutedTextColor, fontSize: 13, lineHeight: 1.4 }}>Latest synced WooCommerce product.</p>}{block.props.showPrice && <p style={{ margin: '0 0 10px', color: theme.primaryColor, fontWeight: 700 }}>{'{{product.price}}'}</p>}{block.props.showButton !== false && <span style={{ display: 'inline-block', background: theme.primaryColor, color: '#ffffff', borderRadius: theme.borderRadius, padding: '8px 12px', fontSize: 13, fontWeight: 700 }}>{block.props.buttonLabel || 'View Product'}</span>}</div>)}</div></div>;
    }
    ...
    return <div style={{ padding: (block.props as { padding?: string }).padding || '8px 0', textAlign: (block.props as { align?: 'left' | 'center' | 'right' }).align || 'left', ...responsiveStyle }} dangerouslySetInnerHTML={{ __html: sanitizeEmailHtml(String(block.props.html || '')) }} />;
}
