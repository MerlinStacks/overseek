...
const VALID_TABS: SupplyChainTab[] = ['suppliers', 'purchasing'];

export function SupplyChainPage() {
    const [searchParams, setSearchParams] = useSearchParams();
    const tabFromUrl = searchParams.get('tab') as SupplyChainTab | null;
    const initialTab = tabFromUrl && VALID_TABS.includes(tabFromUrl) ? tabFromUrl : 'suppliers';
    const [activeTab, setActiveTab] = useState<SupplyChainTab>(initialTab);

    useEffect(() => {
        const currentTabParam = searchParams.get('tab');
        if (activeTab !== 'suppliers' && currentTabParam !== activeTab) {
            const nextParams = new URLSearchParams(searchParams);
            nextParams.set('tab', activeTab);
            setSearchParams(nextParams, { replace: true });
        } else if (activeTab === 'suppliers' && currentTabParam) {
            const nextParams = new URLSearchParams(searchParams);
